...
function paint(){if(!isDoc)return;var slide=document.querySelector(".slide.on");if(!slide)return;var k=slide.querySelector(".k");var rec=window.TEACHER_NOTES&&window.TEACHER_NOTES[k?k.textContent.trim():""];var box=slide.querySelector(".docread");if(!box){box=document.createElement("div");box.className="docread";slide.appendChild(box);}if(!rec){box.innerHTML="";return;}var it=typeof rec==="string"?rec:rec.it;var en=typeof rec==="string"?"":rec.en;function paras(s){return (s||"").split(/(?<=\.)\s+/).filter(Boolean).map(function(x){return "<p>"+x+"</p>";}).join("");}box.innerHTML='<div class="lab">Da leggere \u00b7 Italiano</div>'+paras(it)+'<div class="lab">English</div>'+paras(en);}
